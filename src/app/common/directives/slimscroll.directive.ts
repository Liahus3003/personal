...
export class SlimScrollDirective implements OnInit, OnDestroy {
    ...
    private onWheel(e: MouseWheelEvent): void {
        ...
        // stop window scroll
        if (e.preventDefault && this._isVisible && !this._releaseScroll) {
            e.preventDefault();
        }
    ...
    public scrollContent(y: number, isWheel: boolean, isJump: boolean = false): void {
        this._releaseScroll = false;
        let delta: number = y;
    ...
    private getBarHeight(): void {
        ...
        this._barHeight = Math.max(this._me.offsetHeight / (this._me.scrollHeight === 0 ? 1 : this._me.scrollHeight) *
        this._me.offsetHeight, this._minBarHeight);
