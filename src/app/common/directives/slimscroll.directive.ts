...
interface SlimScrollOptions {
  ...
  // defautlt less class of the slimscroll rail
  railClass: string;

  // defautlt less class of the slimscroll bar
  barClass: string;

  // defautlt less class of the slimscroll wrapper
  wrapperClass: string;
...
export class SlimScrollDirective implements OnInit, OnDestroy {
  ...
  public constructor(private _renderer: Renderer2, elementRef: ElementRef) {
  ...
  private hasParentClass(e: HTMLElement | null, className: string): boolean {
    if (!e) {
      return false;
    }
  ...
  private onWheel(e: WheelEvent): void {
    ...
    // stop window scroll
    if (this._isVisible && !this._releaseScroll) {
      e.preventDefault();
    }
    if (!this._releaseScroll && this._isVisible) {
      e.returnValue = false;
    }
  }
  ...
  private attachWheel(target: Window): void {
    if (!!window.addEventListener) {
      target.addEventListener(
        "DOMMouseScroll",
        (e: Event) => this.onWheel(e as WheelEvent),
        false
      );
      target.addEventListener(
        "mousewheel",
  ...
  public scrollContent(
    ...
    if (isWheel) {
      ...
      // scroll position isn't rounded away when the scrollbar's less is set
  ...
  private refresh(): void {
    ...
    if ("height" in this._options && this._options.height === "auto") {
      ...
      const height =
        this._me && this._me.parentElement
          ? this._me.parentElement.clientHeight
          : 0;
  ...
  private setup(): void {
    ...
    // wrap it
    if (this._me && this._me.parentElement) {
